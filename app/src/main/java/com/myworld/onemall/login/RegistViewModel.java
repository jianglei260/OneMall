...
/**
 * Created by jianglei on 2016/11/28.
 */

public class RegistViewModel extends LoaddingViewModel {
    private static final String TAG = "RegistViewModel";
    ...
    public void signUp() {
        ...
        Observable.create(new Observable.OnSubscribe<Token>() {
            ...
            public void call(Subscriber<? super Token> subscriber) {
                subscriber.onNext(UserRepository.getRepository().signUp(phoneNumber.get(), EncryptUtil.getInstance().encrypt(passwd.get()), smsCode.get(),
            ...
            public void onNext(Token token) {
                ...
                if (token != null && !TextUtils.isEmpty(token.getUserId())) {
                    ...
                    CustomToast.showSuccess(activity, activity.getString(R.string.registe_error));
                }
            }
        });
    }

    public RegistViewModel(RegistActivity activity) {
